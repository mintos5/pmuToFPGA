...
class PMSConf:
    ...
    def add_signal(self, name, power_domain, component):
        if name not in self.signals:
            self.signals[name] = {power_domain: [component]}
        else:
            if power_domain not in self.signals[name]:
                self.signals[name][power_domain] = [component]
            else:
                self.signals[name][power_domain].append(component)

    def add_component(self, name, power_domain):
        self.components[name] = power_domain
